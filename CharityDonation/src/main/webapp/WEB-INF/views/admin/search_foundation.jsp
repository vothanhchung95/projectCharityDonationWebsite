...
<title>Tìm kiếm Nhà tổ chức</title>
<body>

	<div class="mx-5">
		<h2 class="text-center my-5">Tìm kiếm Nhà tổ chức</h2>
		...
		<c:if test="${searchFoundationPagination.size() != 0}">
			<div class="table-responsive shadow">
				<form
					action="<c:url value = "foundation_management/delete_multiple"/>"
					id="deleteForm">
						...
						<tbody>
							<c:forEach var="item" items="${searchFoundationPagination}">
								<tr>
									...
									<td class="col-1"><a
										href='<c:url value = "/admin/foundation_management/edit_foundation/${item.id}"/>'>
											...
											<c:when test="${item.status == 'Active'}">
												<button type="button" class="btn btn-danger"
													...
													data-id="${item.id}"
													onclick="handleDelete('../admin/foundation_management/delete/')">
											...
											<c:when test="${item.status == 'Inactive'}">
												...
												<a
													href='<c:url value = "/admin/foundation_management/undelete/${item.id}"/>'>
					...
					<ul class="pagination justify-content-center">
						<c:if test="${pagination.currentPage > 1}">
							<li class="page-item"><a class="page-link"
								href="<c:url value = "/admin/foundation_management?page=${pagination.currentPage - 1}"/>"><i
							...
							<c:choose>
								<c:when test="${pageNumber == pagination.currentPage}">
									<li class="page-item active"><a class="page-link"
										href="<c:url value = "/admin/foundation_management?page=${pageNumber}"/>">${pageNumber}</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link"
										href="<c:url value = "/admin/foundation_management?page=${pageNumber}"/>">${pageNumber}</a></li>
						...
						<c:if test="${pagination.currentPage < pagination.totalPage}">
							<li class="page-item"><a class="page-link"
								href="<c:url value = "/admin/foundation_management?page=${pagination.currentPage + 1}"></c:url>"><i
		...
		<c:if test="${searchFoundationPagination.size() == 0}">
			<div class="my-5">
				...
				<div class="col-md-3 mx-auto my-5">
					<a href="<c:url value = "/admin/search_foundation"></c:url>">
